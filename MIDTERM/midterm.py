...
def DisplayStatistics(tickets):
    ...
    for dictt in tickets:
        event_id=dictt['event_id'] #checking each the occurence of each event
        if event_id in events:
            events[event_id]+=1 #each time we have the same event_id, its occurence will increment by 1
        else:
            events[event_id]=1 #adding a new event to the dictttionary with occurence 1
